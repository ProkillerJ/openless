...
def patch_user_preferences_default_v8():
    rel = "openless-all/app/src-tauri/src/types.rs"
    text = read(rel)
    start = text.find("impl Default for UserPreferences")
    if start < 0:
        raise RuntimeError("impl Default for UserPreferences not found")
    end = text.find("#[derive(Debug, Clone, Serialize, Deserialize, PartialEq, Eq)]\n#[serde(rename_all = \"camelCase\")]\npub struct ShortcutBinding", start)
    if end < 0:
        raise RuntimeError("end of UserPreferences default impl not found")
    block = text[start:end]
    if "voice_pipelines:" not in block:
        marker = "            active_llm_provider: \"ark\".into(),\n"
        insert = "            voice_pipelines: default_voice_pipelines(),\n            active_voice_pipeline_id: Some(DEFAULT_VOICE_PIPELINE_ID.to_string()),\n"
        if marker not in block:
            raise RuntimeError("active_llm_provider marker not found in UserPreferences default impl")
        block = block.replace(marker, marker + insert, 1)
        text = text[:start] + block + text[end:]
    write(rel, text)


def patch_fake_settings_writer_v8():
    rel = "openless-all/app/src-tauri/src/commands/mod.rs"
    text = read(rel)
    start = text.find("impl SettingsWriter for FakeSettingsWriter")
    if start < 0:
        raise RuntimeError("FakeSettingsWriter impl not found")
    end = text.find("\n    }\n", start)
    if end < 0:
        raise RuntimeError("FakeSettingsWriter impl end not found")
    impl_block = text[start:end]
    if "refresh_voice_pipeline_hotkeys" not in impl_block:
        marker = "        fn refresh_coding_agent_hotkey(&self) {"
        insert = "        fn refresh_voice_pipeline_hotkeys(&self) {}\n\n"
        pos = text.find(marker, start)
        if pos < 0:
            raise RuntimeError("refresh_coding_agent_hotkey marker not found")
...
def main():
    ...
    patch_user_preferences_default_v8()
    patch_fake_settings_writer_v8()
