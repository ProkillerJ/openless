...
def patch_coordinator_helpers_v5():
    rel = "openless-all/app/src-tauri/src/coordinator.rs"
    text = read(rel)
    if "fn ensure_asr_credentials_for_provider" in text:
        write(rel, text)
        return
    old = """fn ensure_asr_credentials() -> Result<(), String> {
    let active_asr = CredentialsVault::get_active_asr();

    // 本地 Qwen3-ASR 没有\"凭据\"概念，但需要：(a) macOS 平台 (b) 模型已下载。
"""
    new = """fn ensure_asr_credentials() -> Result<(), String> {
    let active_asr = CredentialsVault::get_active_asr();
...
"""
    if old not in text:
        raise RuntimeError("ensure_asr_credentials coordinator overload anchor not found")
    text = text.replace(old, new, 1)
    write(rel, text)


def patch_coordinator_followups_v5():
    rel = "openless-all/app/src-tauri/src/coordinator.rs"
    text = read(rel)
...
def main():
    ...
    old.patch_coordinator_helpers_rs = patch_coordinator_helpers_v5
    old.patch_all()
    ...
    patch_coordinator_followups_v5()
