...
def patch_user_preferences_default_v9():
    rel = "openless-all/app/src-tauri/src/types.rs"
    text = read(rel)
    anchor = "impl Default for UserPreferences {\n"
    start = text.find(anchor)
    if start < 0:
        raise RuntimeError("exact impl Default for UserPreferences not found")
    end_anchor = "#[derive(Debug, Clone, Serialize, Deserialize, PartialEq, Eq)]\n#[serde(rename_all = \"camelCase\")]\npub struct ShortcutBinding"
    ...
    if "voice_pipelines:" not in block:
        ...
        if marker not in block:
            raise RuntimeError("active_llm_provider marker not found in exact UserPreferences default impl")
...
def main():
    ...
    patch_user_preferences_default_v9()
